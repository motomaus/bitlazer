...
describe("StakeLBTC Contract", function () {
  ...
  it("Should allow staking and minting tokens", async function () {
    ...
    await stakeContract
      .connect(addr1Signer)
      .stake(stakeAmount, { value: stakeAmount });
  ...
  it("Should distribute rewards proportionally to stakers", async function () {
    ...
    await stakeContract
      .connect(addr1Signer)
      .stake(stakeAmount1, { value: stakeAmount1 });
    await stakeContract
      .connect(addr2Signer)
      .stake(stakeAmount2, { value: stakeAmount2 });
    ...
    await stakeContract
      .connect(ownerSigner)
      .charge(rewardAmount, { value: rewardAmount });
  ...
  it("Should fail unstaking if balance is insufficient", async function () {
    ...
    await expect(
      stakeContract.connect(addr1Signer).unstake(unstakeAmount),
    ).to.be.revertedWith("Insufficient balance to unstake");
  });
  ...
  it("Should update version only by the owner", async function () {
    ...
    await expect(
      stakeContract.connect(addr1Signer).setVersion(newVersion),
