...
const MainLayout = () => {
  ...
  const linesRef2 = useRef<HTMLDivElement[]>([])
  ...
  const getRandomTop = () => {
    return `${Math.random() * (70 - 10) + 10}vh`
  }
  const getRandomTop2 = () => {
    return `${Math.random() * (70 - 15) + 15}vh`
  }

  useEffect(() => {
    const randomTop = getRandomTop() // Генерируем случайное значение для top один раз
    const randomTop2 = getRandomTop2()
    linesRef.current.forEach((line) => {
      const initialRotate = parseFloat(line.dataset.rotate || '0')
      const randomOffset = Math.random() * 1 - 1 // Случайный сдвиг от -2 до 2 градусов
  ...
  const handleMouseLeave = () => {
    setShowDot((prev) => ({ ...prev, visible: false }))
  }
    ...
    <div className="relative bg-black min-h-screen flex flex-col overflow-hidden md:pointer-events-none md:[&_*]:pointer-events-none">
      <div className="w-full h-full absolute top-0 left-0">
        ...
        <div
          ...
          className="w-[170vw] h-[3px] py-[2px] left-0 !absolute origin-top-left rotate-line hover-area"
          data-rotate="-17.62"
          onMouseEnter={(e) => handleMouseEnter(e, -17.62)}
          onMouseMove={(e) => handleMouseMove(e, -17.62)}
          onMouseLeave={handleMouseLeave}
        >
          <div className="w-full  h-[0.9px] bg-[#68d861] absolute top-0 left-0"></div>
        </div>
        ...
        <div
          ref={(el) => el && linesRef2.current.push(el)}
          className="w-[170vw] h-[3px] py-[2px] right-0 !absolute origin-top-right rotate-line hover-area"
          data-rotate="-2.23"
          onMouseEnter={(e) => handleMouseEnter(e, -2.23)}
          onMouseMove={(e) => handleMouseMove(e, -2.23)}
          onMouseLeave={handleMouseLeave}
        >
          <div className="w-full  h-[0.9px] bg-[#68d861] absolute top-0 right-0"></div>
        </div>
        <div
          ref={(el) => el && linesRef2.current.push(el)}
          className="w-[170vw] h-[3px] py-[2px] right-0 !absolute origin-top-right rotate-line hover-area"
